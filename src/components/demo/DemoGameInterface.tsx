...
/**
 * Demo candlestick chart component - completely independent from Aptos version.
 * Pure demo mode for testing and showcasing game mechanics without blockchain.
 */
const DemoGameInterface = () => {
    const dbg = useDebug();
    const chartRef = useRef<HTMLDivElement>(null!);
    const p5InstanceRef = useRef<p5 | null>(null);
    const modalOpenRef = useRef(false);

    // Demo game state - completely separate from Aptos
    const [currentPrice, setCurrentPrice] = useState(0);
    const [balance, setBalance] = useState(1000); // Demo starts with $1000
    ...
    useP5Chart({
        ...
        aptosMode: false // Demo mode - no Aptos integration
    });
    ...
    return (
        ...
        >
            ...
            <Footer
                balance={balance}
                walletBalance={1000} // Fixed demo wallet balance
                isHolding={isHolding}
                showLiquidation={showLiquidation}
                rugpullType={rugpullType}
                isModalOpen={isModalOpen}
                setIsModalOpen={setIsModalOpen}
                aptosMode={false} // Demo mode = no Aptos
                gameState={'waiting'} // Always waiting in demo
            />
        </div>
    );
};
